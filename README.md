...
- **An interactive web app prototype (Streamlit):** `app.py` provides a fully functional Streamlit application that serves as the interactive web prototype.
    - **Chart(s) showing real energy data:** The app displays a dynamic line chart of historical generation.
    - **AI-generated summary (mock):** The summary generated by `summary_module.py` is displayed directly within the Streamlit interface.
    - **Anomaly or classification markers clearly labeled:** Anomalies detected by `ai_module.py` are plotted as distinct red dots on the historical trend chart within the app. A table of detected anomalies is also displayed.
- **Automated alert trigger using Zapier:** The `app.py` script, upon detecting anomalies, generates an `alerts_today.csv` file. This file can be used as a trigger for a Zapier workflow (e.g., monitoring a synced Google Drive folder) to send automated emails or Slack messages.
- **A working codebase:** All Python files (`app.py`, `ai_module.py`, `summary_module.py`) are provided and work together to deliver the described functionality.
- **Documentation covering:** (This `README.md` serves this purpose)
...
Step 3: Run the Analysis
 - After reviewing the raw data preview, locate the large "🚀 Run Analysis" button on the web page.
